...
begin
 ...
 with ArchiveStream do begin
  ...
  Read(Dir,SizeOf(Dir)); // читаем первый оффсет. он нам понадобитс€ дл€ вычислени€ примерного количества файлов
  
  RecordsCount := (Dir - SizeOf(Hdr)) div 4;

  Seek(-4,soCurrent); // откатываемс€ назад
...
{*}Progress_Pos(i);
  ...
  RecordsCount := RecordsCount - 1; //пр€чем eof-запись от GUI ^3^

...
type
  ...
  TCrossnetDir = packed record
   NameOffset : longword; // относительное смещение в таблице имени с которого начинается имя
   FileOffset : longword; // абсолютное смещение
   FileSize : longword; // имя файла
  end;
 ...
 with ArchiveStream do begin
  ...
  with Hdr do begin
   ...
   if (FileCount = 0) or
      (FileCount > $FFFF) or
      (NameTableLen = 0) or
      (NameTableLen > (Size div 10)) then Exit; // div 10 значит если больше десятой части архива
